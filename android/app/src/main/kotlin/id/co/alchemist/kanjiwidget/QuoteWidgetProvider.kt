...
class QuoteWidgetProvider : HomeWidgetProvider() {
        ...
        appWidgetIds.forEach { widgetId ->
            val views = RemoteViews(context.packageName, R.layout.quote_widget).apply {
                val pendingIntent = HomeWidgetLaunchIntent.getActivity(
                    ...
                    Uri.parse("kanjiwidget://edit?widgetId=$widgetId&kind=quote"),
                )
                setOnClickPendingIntent(R.id.quote_widget_root, pendingIntent)

                val imagePath = widgetData.getString("quote_image_$widgetId", null)
                    ?: widgetData.getString("quote_image", null)
                if (imagePath != null) {
                    setImageViewBitmap(
                        R.id.quote_widget_image,
    ...
    override fun onDisabled(context: Context) {
        ...
        QuoteAlarmScheduler.cancel(context)
    }
}
